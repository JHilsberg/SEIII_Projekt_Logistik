...
class FormController  extends BaseController
{
    ...
    // abschicken oder speichern einer bearbeiteten Order

    public function editSubmitted($orderid){

        if (Input::get('submit')) {
            return $this->validateSubmitEdit($orderid);
        } elseif (Input::get('save')) {
            return  $this->validateSaveEdit($orderid);
        }
    ...
    public function validateSubmitEdit($orderid)
    {
        ...
        if ($validator->fails()) {
            return Redirect::to('edit/'.$orderid)
                ->withInput()
            ...
            return $this->submitEdit($orderid);
    ...
    public function submitEdit($orderid)
    {
        $order = Order::find($orderid);
        ...
        $abholadresse = Adress::find($order->abholadresse()->first()->id);
        ...
        $lieferadresse = Adress::find($order->lieferadresse()->first()->id);
...
//        $order->lieferadresse = $lieferadresse->id;
    ...
    public function validateSaveEdit($orderid)
    {
        date_default_timezone_set('Europe/Berlin');
    ...
    //normales Speichern oder Abschicken einer neuen Order
